...
-export_type(['test_import'/0]).

-spec encode_msg(#test_import{}) -> binary().
encode_msg(Msg) when tuple_size(Msg) >= 1 -> encode_msg(Msg, element(1, Msg), []).

-spec encode_msg(#test_import{}, atom() | list()) -> binary().
...
encode_msg(Msg, MsgName, Opts) ->
    ...
    case MsgName of test_import -> encode_msg_test_import(id(Msg, TrUserData), TrUserData) end.
...
merge_msgs(Prev, New, MsgName, Opts) ->
    TrUserData = proplists:get_value(user_data, Opts),
    case MsgName of test_import -> merge_msg_test_import(Prev, New, TrUserData) end.
...
get_msg_defs() -> [{{msg, test_import}, [#field{name = ui32, fnum = 1, rnum = 2, type = uint32, occurrence = optional, opts = []}]}].


get_msg_names() -> [test_import].


get_group_names() -> [].


get_msg_or_group_names() -> [test_import].
...
get_all_source_basenames() -> ["test_i.proto"].
