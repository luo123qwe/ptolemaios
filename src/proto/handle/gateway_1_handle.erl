...
handle(#gateway_c_login{account = MsgAccount}, #gateway_state{account = Account} = GateWay) ->
    ...
    %% 是否被锁定
    ?EC_NOT_MATCH3(plm_ll:lock(?LOCAL_LOCK_ACCOUNT1(MsgAccount)), lock, ?EC_OTHER_HAD_LOGIN),
    %% 锁住该账号, 防止并发
...
handle(#gateway_c_select_role{id = MsgPlayerId}, #gateway_state{account = Account, player_id = PlayerId} = GateWay) ->
    ...
    ?EC_NOT_MATCH3(vsql:lookup_ets(player, [MsgPlayerId]),
        #player{}, ?EC_NO_ROLE),
    %% 先锁定这个id
    case plm_ll:lock(?LOCAL_LOCK_PLAYER_ID1(MsgPlayerId)) of
        lock ->
            %% 出错保证不死锁
            try
                ...
                _:_ ->
                    plm_ll:release(?LOCAL_LOCK_PLAYER_ID1(MsgPlayerId)),
                    ?THROW_EC1(?EC_ERROR)
            end;
        _ ->% 并发
