...
%%% @doc 遍历操作
%%%
%%% 可以通过返回`?FOLD_BREAK和?FOLD_BREAK1'实现break操作
%%%
%%% `?PTOLEMAIOS_MASK_COPY_FROM_ERL'标记了哪些代码从erl源码复制过来
%%% @end
%%%-------------------------------------------------------------------
-module(fold).
-author("dominic").
...
list_r_1(F, Acc, [H | T]) ->
    case list_r_1(F, Acc, T) of
        ?FOLD_BREAK -> ?FOLD_BREAK1(Acc);
        ?FOLD_BREAK1(_Acc1) = Break -> Break;
        Acc1 ->
            case F(H, Acc1) of
                ?FOLD_BREAK -> ?FOLD_BREAK1(Acc1);
                ?FOLD_BREAK1(_Acc2) = Break -> Break;
                Acc2 -> Acc2
            end
    end.
            ...
            case F(Key, Value, Acc1) of
                ...
                Acc2 ->
                    case gb_trees_l_1(F, Acc2, Big) of
                        ?FOLD_BREAK -> ?FOLD_BREAK1(Acc2);
                        ?FOLD_BREAK1(_Acc3) = Break -> Break;
                        Acc3 -> Acc3
                    end
            end
    end;
