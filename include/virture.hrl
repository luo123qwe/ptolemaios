...
-record(vmysql, {
    ...
    select_key = error({require, select_key}) :: [],% 数据库搜索全部数据用的key
    private_key = error({require, private_key}) :: [],% 数据的主键
    ...
    %% 同步策略, 可以同时存在, 互相独立
    sync_time = 10 :: integer(),% 上次同步后N秒再次检查同步
    sync_size = 10 :: integer(),% 变化数据达到N条同步
